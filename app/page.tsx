...
export default function LandingPage() {
  ...
  const reviews = [
    ...
    },
    {
      ...
      text: "사업하느라 바빠서 아무나 만날 시간이 없었습니다. 확실하게 신원이 검증된 분, 저와 결이 맞는 분을 알아서 필터링해주니 시간 낭비가 전혀 없네요. 지인 차단 기능 덕분에 마음 편히 이용 중입니다."
    },
    {
      ...
      text: "가입비가 없어서 반신반의했는데, 막상 진행해보니 결정사보다 훨씬 프라이빗하고 매니저님이 세심합니다. 상호 호감이 확인되고 매칭 성사 시에만 결제하는 시스템이 정말 합리적이에요."
    },
    {
  ...
  const renderHome = () => (
    <div className="animate-in fade-in duration-500">
      ...
      {/* 🚀 SSOK Guarantee (만남 보장) - 숫자 디자인 분리 적용 */}
      ...
      <section className="py-28 bg-[#FDFBFB] overflow-hidden">
        ...
        <div className="relative w-full">
          ...
          <div className="animate-marquee gap-6 px-6">
            {/* 자연스러운 무한 루프를 위해 배열을 2번 반복 (5개 + 5개 = 총 10개 카드) */}
    ...
    <div className="pt-36 pb-28 px-6 max-w-5xl mx-auto animate-in slide-in-from-right-8 duration-500">
      <div className="text-center mb-20">
        ...
        <p className="text-[16px] md:text-[18px] text-[#8C7A7D] leading-[1.7] break-keep font-medium max-w-2xl mx-auto">
          ...
          SSOK은 국가 발급 증명서와 사원증 등 법적 효력이 있는 서류만을 취급하여<br className="hidden md:block"/>
          가장 확실하고 안전한 만남만을 주선합니다.
        </p>
      </div>
      ...
      <div className="bg-[#FAFAFA] border border-[#F0EBEB] rounded-3xl p-8 md:p-10 flex flex-col md:flex-row items-center gap-8 mb-24 shadow-sm">
        ...
        <div>
          ...
          <p className="text-[#8C7A7D] text-[15px] md:text-[16px] leading-[1.6] break-keep">
            SSOK은 회원의 개인정보 보호를 최우선으로 합니다. 검증을 위해 제출하신 모든 민감 서류는 전담 매니저의 <span className="font-bold text-[#FF2E63]">확인 즉시 시스템에서 영구적으로 파기</span>되며, 외부로 절대 유출되지 않으니 안심하세요.
          </p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        {[
          { icon: <UserCheck className="w-6 h-6 text-white"/>, title: "STEP 01. 본인 및 혼인 여부 인증", desc: "통신사 본인 인증을 통한 실명 확인은 물론, 혼인관계증명서(상세)를 필수적으로 검토하여 법적으로 완벽한 싱글(미혼/돌싱)임을 교차 검증합니다." },
          { icon: <Briefcase className="w-6 h-6 text-white"/>, title: "STEP 02. 직장 및 직업 인증", desc: "명함만으로는 부족합니다. 사원증, 건강보험자격득실확인서, 재직증명서, 전문직 자격증명원 등 확실한 증빙 서류를 요구합니다." },
          { icon: <GraduationCap className="w-6 h-6 text-white"/>, title: "STEP 03. 학력 인증", desc: "대학교 또는 대학원의 졸업증명서 원본 서류를 통해 프로필에 기재된 학력의 진위 여부를 꼼꼼하게 대조합니다." },
          { icon: <Building2 className="w-6 h-6 text-white"/>, title: "STEP 04. 자산 및 소득 인증 (선택)", desc: "근로소득원천징수영수증, 부동산 등기부등본, 고급 차량등록증 등을 매니저에게 제출하여 '상위 1% 인증 배지'를 부여받을 수 있습니다." },
          { icon: <FileText className="w-6 h-6 text-white"/>, title: "STEP 05. 매니저 최종 인터뷰", desc: "서류 통과 후 전담 매니저와의 상담을 통해 가입 목적의 건전성, 기본적인 매너, 대화의 결을 최종 평가하여 승인합니다." }
        ].map((item, idx) => (
      ...
      <div className="bg-[#322729] rounded-[3rem] p-10 md:p-16 text-white mb-24 shadow-xl overflow-hidden relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
          <div className="border-b md:border-b-0 md:border-r border-white/10 pb-10 md:pb-0 md:pr-12">
            <Cpu className="w-12 h-12 text-white/30 mb-6" />
            <h3 className="text-[22px] font-black mb-4 text-white/50 line-through">일반 데이팅 앱 (AI 알고리즘)</h3>
            <ul className="space-y-4 text-[15px] text-white/60 leading-relaxed break-keep">
              <li>• 나이, 키, 직업 등 겉으로 보이는 숫자로만 필터링합니다.</li>
              <li>• 무작위로 여러 명을 띄워주고 선택을 강요합니다.</li>
              <li>• 상대방의 실제 성격이나 대화 매너는 만나보기 전까지 절대 알 수 없습니다.</li>
            </ul>
          </div>
